...
namespace SharpAESCrypt_ZZZ
{
    ...
    public class SharpAESCrypt : Stream
    {
        ...
        public static byte[] EncryptArray(byte[] normalTextBytes, string password)
        {
            var normalStream = new MemoryStream(normalTextBytes);
            var encryptedStream = new MemoryStream();
            Encrypt(password, normalStream, encryptedStream);
            var encryptedBytes = encryptedStream.ToArray();
            return encryptedBytes;
        }

        public static byte[] DecryptArray(byte[] encryptedData, string password)
        {
            var encryptedStream = new MemoryStream(encryptedData);
            var normalStream = new MemoryStream();
            Decrypt(password, encryptedStream, normalStream);
        ...
        private static bool Unittest(string message, MemoryStream input)
        {
            ...
            const string PASSWORD_CHARS = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!\"#آ¤%&/()=?`*'^آ¨-_.:,;<>|";
            const int MIN_LEN = 1;
            ...
            {
                //TODO: Hashvأ¦rdien skal fأ¦rdigأ¸res fأ¸r hash vأ¦rdien hentes?
